...
async def reg_link(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("💰 **Step 2/4:** Enter Prize Amount (e.g., 10.000):")
    return PRIZE
...
async def handle_notice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"{preview_text}\n"
        f"───────────────────\n\n"
        f"Click 'Publish Both Posts' to go live!",
        reply_markup=InlineKeyboardMarkup(keyboard),
        parse_mode="Markdown"
    )
    return CONFIRM
...
async def publish_giveaway(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cursor.execute("""
            INSERT INTO giveaways (prize, task, link, notice) 
            VALUES (?, ?, ?, ?)
        """, (
            prize_val,
            context.user_data.get('ga_task', ''),
            context.user_data.get('ga_link', ''),
            context.user_data.get('ga_notice', '')
        ))
        ...
        cursor.execute("""
            UPDATE giveaways 
            SET message_id = ?, chat_id = ?, main_post_id = ?, counter_post_id = ? 
            WHERE giveaway_id = ?
        ...
        await query.message.reply_text(
            ...
            f"📝 **Main Post:** https://t.me/SabbirGA/{main_msg.message_id}\n"
            f"📊 **Counter Post:** https://t.me/SabbirGA/{counter_msg.message_id}"
        )
...
async def handle_proof(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await update.message.reply_text(
            f"✅ **Proof Submitted!**\n\n"
            f"📸 Your proof has been submitted for Giveaway #{giveaway_id}."
        )
...
async def draw_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cursor.execute("""
            SELECT g.giveaway_id, g.prize, 
                   COUNT(p.user_id) as entry_count
            FROM giveaways g 
        ...
        for g in giveaways:
            if g['entry_count'] > 0:
                keyboard.append([InlineKeyboardButton(
                    f"🎯 #{g['giveaway_id']} - {g['prize']} ({g['entry_count']} entries)",
                    callback_data=f"draw_now_{g['giveaway_id']}"
                )])
        ...
        await query.message.reply_text(
            "🎯 **Select a giveaway to draw winner:**",
            reply_markup=InlineKeyboardMarkup(keyboard),
            parse_mode="Markdown"
        )
...
async def draw_winner(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        cursor.execute("""
            UPDATE giveaways 
            SET status = 'ended', winner_id = ?, winner_username = ? 
            WHERE giveaway_id = ?
        ...
        announce_text = (
            ...
            f"**🎊 Congratulations!**\n"
            f"{winner_mention}\n\n"
            f"Please contact @SABBIRXDM to claim your prize! 🎁"
        )
...
async def view_statistics(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        cursor.execute("""
            SELECT g.giveaway_id, g.prize, g.status, 
                   COUNT(p.user_id) as entry_count,
                   COUNT(CASE WHEN p.proof_message_id IS NOT NULL THEN 1 END) as proof_count,
                   g.winner_id, g.winner_username
            FROM giveaways g 
            LEFT JOIN participants p ON g.giveaway_id = p.giveaway_id 
            GROUP BY g.giveaway_id 
            ORDER BY g.giveaway_id DESC
        ...
        for r in rows:
            ...
            stats_text += f"Status: {status_icon} `{r['status']}`\n"
            stats_text += f"👥 Entries: **{r['entry_count']}** | 📸 Proofs: **{r['proof_count']}**\n"
            stats_text += f"{winner_text}\n\n"
...
async def test_channel_connection(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        await query.message.reply_text(
            ...
            f"📌 Bot can send messages to @SabbirGA"
        )
...
def main():
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            ...
            NOTICE: [
                MessageHandler(filters.TEXT & ~filters.COMMAND, handle_notice),
                CallbackQueryHandler(skip_notice, pattern="^skip_notice$")
            ],
            CONFIRM: [CallbackQueryHandler(publish_giveaway, pattern="^publish_now$")]
        },
        fallbacks=[
            CallbackQueryHandler(cancel_flow, pattern="^panel_cancel$"),
